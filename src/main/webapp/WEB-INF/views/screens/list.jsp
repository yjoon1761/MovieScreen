...
<head>
  ...
  <title>MovieScreen - ScreenList</title>
...
<body>
	...
	<section>
		...
		<form action="scSearch" method="get">
				<!-- 검색 카테고리 -->
				<select name="category">
					<option value="THNAME">영화관 지점</option>
					<option value="SCNAME">상영관 이름</option>
					<option value="SCTYPE">상영타입</option>
					<option value="SCTOTAL">좌석 수</option>
				</select>
		...
		<tr>
			...
			<c:forEach var="list" items="${screensList}">
				<tr>
					<td><a href="scView?scNo=${list.scNo}">${list.thName}</a></td>
					<td>${list.scName}</td>
					<td>${list.scType}</td>
					<td>${list.scTotal}</td>
				</tr>
...
<script>
	$('#limit').change(()=>{
		...
		location.href = "scList?limit=" + limit;
	});
</script>
</html>